...
import { getChannel, getChannelModels, searchChannels } from '../../channels/api'
import {
...
/**
 * Bulk editor for one model's per-channel ratios — the model-first mirror
 * of ChannelModelRatiosDialog. Lists every channel that declares support
 * for modelName; a checked row with a ratio becomes (or updates) a
 * binding on save, an unchecked row that was previously bound is removed.
 */
function ModelChannelRatiosDialog({
                                      open,
                                      onOpenChange,
                                      userId,
                                      modelName,
    ...
    const { t } = useTranslation()
    const [rows, setRows] = useState<ChannelRatioRow[]>([])

    const { data: channels = [], isLoading: channelsLoading } = useQuery({
        queryKey: ['user-channel-bindings-channels-for-model', modelName],
        queryFn: async () => {
            const res = await searchChannels({ model: modelName, page_size: 50 })
            return res.data?.items ?? []
        },
        enabled: open,
    })

    // (Re)seed the editable rows whenever the dialog opens for a model:
    // channels this user already binds modelName to start checked with
    // their saved ratio, every other channel supporting modelName starts
    // unchecked at a default of 1.
    useEffect(() => {
        if (!open) return
        const existingByChannel = new Map(
            existingBindings.map((b) => [b.channel_id, b.ratio])
        )
        setRows(
            channels.map((ch) => ({
                channelId: ch.id,
                channelName: ch.name,
                enabled: existingByChannel.has(ch.id),
                ratio: String(existingByChannel.get(ch.id) ?? 1),
            }))
        )
    }, [open, channels, existingBindings])

    const saveMutation = useMutation({
        mutationFn: async () => {
            const wasBound = new Set(existingBindings.map((b) => b.channel_id))
            await Promise.all(
                rows.map((row) => {
                    const ratio = parseFloat(row.ratio) || 1
                    if (row.enabled) {
                        return upsertUserChannelBinding(
                            userId,
                            row.channelId,
                            modelName,
                            ratio
                    ...
                    if (wasBound.has(row.channelId)) {
                        return deleteUserChannelBinding(userId, row.channelId, modelName)
                    }
                    return Promise.resolve()
                })
            )
        ...
        onSuccess: () => {
            toast.success(t('Channel ratios saved'))
            onOpenChange(false)
            onSaved()
        },
        onError: (e) => {
            toast.error(
                e instanceof Error ? e.message : t('Failed to save channel ratios')
            )
        },
    })
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className='sm:max-w-md'>
                ...
                <DialogFooter>
                    ...
                    <Button
                        type='button'
                        disabled={saveMutation.isPending || channelsLoading}
                        onClick={() => saveMutation.mutate()}
                    >
                        {saveMutation.isPending && (
                            <Loader2 className='mr-1.5 h-3.5 w-3.5 animate-spin' />
...
function ChannelBindingsPanel({ userId }: { userId: number }) {
    ...
    // Bind-a-model entry point: model names actually configured on some
    // channel, read directly from the channels table's own Models field
    // (not the abilities index, and not a separately-curated model catalog
    // that may well be empty in a fresh setup) — picking a model opens
    // ModelChannelRatiosDialog, which lists every channel that supports it.
    // Fetched once and filtered client-side since it's a flat name list, not
    // worth a per-keystroke round trip.
    const { data: allEnabledModels = [] } = useQuery({
        queryKey: ['user-channel-bindings-enabled-models'],
        queryFn: async () => {
            const res = await getChannelModels()
            return res.data ?? []
        },
    })
    const modelSearchResults = useMemo(() => {
        ...
        if (!term) return []
        return allEnabledModels.filter((m) => m.toLowerCase().includes(term))
    }, [allEnabledModels, modelSearchTerm])

    const boundModelNames = useMemo(
        () => new Set(bindings.map((b) => b.model_name)),
        [bindings]
    )
    ...
    return (
        <div className='space-y-3'>
            ...
            <div className='border-border/60 space-y-2 rounded-md border border-dashed p-2.5'>
                <Popover
                    open={modelSearchTerm.trim().length > 0}
                    onOpenChange={(open) => {
                        if (!open) setModelSearchTerm('')
                    }}
                >
                    <PopoverTrigger asChild>
                        <Input
                            placeholder={t('Search model by name...')}
            ...
            {editingModel && (
                <ModelChannelRatiosDialog
                    ...
                    existingBindings={bindings.filter(
                        (b) => b.model_name === editingModel
                    )}
                    onSaved={() => {
                        queryClient.invalidateQueries({ queryKey: bindingsQueryKey })
                    }}
