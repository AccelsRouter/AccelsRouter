...
const (
	...
	PaymentProviderWonderGate   = "wondergate"
)
...
// RechargeWonderGate credits a WonderGate top-up on a successful notification.
// Idempotent and row-locked.
func RechargeWonderGate(tradeNo string, callerIp string) (err error) {
	if tradeNo == "" {
		return errors.New("未提供支付单号")
	}
	...
	if err != nil {
		common.SysError("wondergate topup failed: " + err.Error())
		return errors.New("充值失败，请稍后重试")
	}

	if quotaToAdd > 0 {
		RecordTopupLog(topUp.UserId, fmt.Sprintf("WonderGate充值成功，充值额度: %v，支付金额: %.2f", logger.FormatQuota(quotaToAdd), topUp.Money), callerIp, topUp.PaymentMethod, PaymentMethodWonderGate)
	}
